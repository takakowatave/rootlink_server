...
function buildSenseTranslationPrompt(items: TranslationSourceItem[]): string {
  return [
    ...
    "",
    "CRITICAL — definitionJa ↔ exampleJa alignment (highest priority):",
    "- The Japanese gloss you chose for definitionJa MUST appear verbatim as a substring inside exampleJa.",
    "  This lets the learner map the headword they are studying to its appearance in the example.",
    "- This alignment is MORE IMPORTANT than natural-sounding Japanese. Prefer a slightly awkward sentence with the correct gloss over a smooth sentence with a synonym.",
    "- Never substitute a synonym or near-synonym of definitionJa in exampleJa, even if the synonym is more natural in context.",
    "  e.g. definitionJa = 「急を要する」, exampleEn = 'the urgent demand for more funding'",
    "       -> exampleJa MUST contain 「急を要する」 (e.g. 「より多くの資金を求める、急を要する要求」),",
    "          NOT 「緊急の要求」 even if 「緊急の」 sounds more natural.",
    "  e.g. definitionJa = 「訂正」, exampleEn = 'she made a small correction to the report'",
    "       -> exampleJa MUST contain 「訂正」 (e.g. 「彼女はレポートに小さな訂正を加えた」),",
    "          NOT 「彼女はレポートを少し直した」.",
    "- If the definitionJa has a grammatical suffix that does not fit the example slot (e.g. verb-form gloss in an adjectival slot), REPHRASE definitionJa first so it can appear as a plain substring. Do not diverge in exampleJa.",
    "- Self-check before returning each item: is definitionJa a substring of exampleJa? If not, rewrite exampleJa (or definitionJa) until it is.",
    "",
    "Return JSON only.",
