...
function buildSenseTranslationPrompt(items: TranslationSourceItem[]): string {
  return [
    ...
    "",
    "definitionJa ↔ exampleJa の整合:",
    "- definitionJa は上記ルール通り、日本人学習者が期待する辞書見出し級の短い自然な訳語であること。",
    "  (measure → 「測る」「評価する」/ oppose → 「反対する」/ postpone → 「延期する」 のような短い訳語。定義文パラフレーズは不可)",
    "- exampleJa は例文の自然な日本語訳。definitionJa の訳語が例文中に自然に現れるように訳す。",
    "  活用・語尾変化・語順変化は自由 (e.g. 「延期する」→「延期された」「延期しよう」など)。文字レベルの完全一致は不要。",
    "- 自然な日本語を優先。definitionJa の語がその活用形で例文の意味に自然に収まらない場合は、",
    "  それは definitionJa 自体が定義文になっているサイン。まず definitionJa をより短い自然な訳語に直すこと。",
    "  例文訳を歪めて定義文を無理に押し込むのは禁止。",
    "  e.g. exampleEn = 'she made a small correction to the report'",
    "       -> definitionJa = 「訂正」, exampleJa = 「彼女はレポートに小さな訂正を加えた」 が自然。",
    "  e.g. exampleEn = 'she felt no regret at his death'",
    "       -> definitionJa = 「後悔」ではなく「悔やみ」等の自然な訳、exampleJa = 「彼女は彼の死に何の悔やみも感じなかった」 のように自然に。",
    "",
    "Return JSON only.",
